...
export class ContentVideoComprehensionTestsGeminiClient {
  async generateTests(
    ...
    const prompt = buildAiPrompt(
      ...
      DEFAULT_PROMPT_COMPREHENSION_TESTS,
      {
        EXPECTED_TEST_COUNT: String(EXPECTED_TEST_COUNT),
        KEY_VOCAB_COUNT: String(KEY_VOCAB_COUNT),
        MCQ_COUNT: String(EXPECTED_TEST_COUNT - 1),
        WEAK_SPOTS: weakSpots,
        LEARNING_GOAL: input.learningGoal,
        TIME_TO_ACHIEVE: input.timeToAchieve,
        HOBBIES: hobbyLine,
        VOCAB_STRETCH_INSTRUCTION: stretch.instruction,
        VOCAB_TARGET_BAND: stretch.vocabularyTargetBand,
        LEARNER_LEVEL: level,
        VOCABULARY_TERMS: vocabList,
        VIDEO_THEMES: videoThemes,
