...
function normalize_apostrophes(raw: string): string {
  return raw.replace(/[\u2019`´]/g, "'");
}

function is_probable_proper_noun(word: string): boolean {
...
function lemmatize_token(token: string): string {
  let low = normalize_apostrophes(token).toLowerCase();
  const headword = SINGLE_TOKEN_HEADWORDS[low];
  if (headword) {
    return headword;
  }
  if (IRREGULAR_LEMMAS[low]) {
    return IRREGULAR_LEMMAS[low];
  }
  ...
  if (low.endsWith("es") && low.length > 4) {
...
function sanitize_phrase(raw: string): string {
  const parts = raw.split(/\s+/).filter((p) => p.length > 0);
  return parts.map((part) => lemmatize_token(part)).join(" ");
}

/**
 * Returns a cleaned vocabulary term or null when the item should be dropped (e.g. a name).
 */
export function sanitizeVocabularyTerm(raw: string): string | null {
  const trimmed = normalize_apostrophes(raw).trim().replace(/\s+/g, " ");
  if (trimmed.length < 2 || trimmed.length > 96) {
    return null;
  }
  if (is_probable_proper_noun(trimmed)) {
    return null;
  }
  const singleKey = trimmed.toLowerCase();
  const contractionHead = SINGLE_TOKEN_HEADWORDS[singleKey];
  if (contractionHead) {
    return contractionHead;
  }
  const sanitized = sanitize_phrase(trimmed);
...
export function dedupeSanitizedVocabularyTerms(words: readonly string[]): string[] {
